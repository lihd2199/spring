...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	 ...
	 * //		忽略给定接口的自动装配功能
	 * //		当A中有属性B，那么当Spring在获取A的Bean的时候如果其属性B还没有初始化，那么Spring会自动初始化B，
	 * //		这也是Spring中提供的一个重要特性。
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		try {
			//验证及准备覆盖的方法
			//针对配置 lookup-method 和 replace-method
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
		...
		if (instanceWrapper == null) {
			//根据bean指定的策略，创建新的实例，将beanDefinition转换为BeanWrapper
		...
		//是否需要提早曝光：单例 && 允许循环依赖&& 当前bean正在创建中（getSingleton中记录）
		//在Spring中会有循环依赖的情况，例如，当A中含有B的属性，而B中又含有A的属性时就会构成一个循环依赖，
		// 此时如果A和B都是单例，那么在Spring中的处理方式就是当创建B的时候，
		// 涉及自动注入A的步骤时，并不是直接去再次创建A，而是通过放入缓存中的ObjectFactory来创建实例，
		// 这样就解决了循环依赖的问题。
...
//			});
		...
		try {
			//对bean进行填充，将各个属性注入，其中可能存在依赖于其他bean属性，则会递归初始化依赖bean
		...
		if (earlySingletonExposure) {
			...
			if (earlySingletonReference != null) {
				...
				if (exposedObject == bean) {
					...
					//actualDependentBeans 不为空则表示当前bean创建后，其依赖的bean却没有全部创建完，也就是说存在循环依赖
	 ...
	 *  aop就是在这里将advice织入bean中的，若没有则直接返回bean，不做任何处理
	 *
	 */
